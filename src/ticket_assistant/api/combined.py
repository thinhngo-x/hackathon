...
async def classify_and_send_report(
    ...
    try:
        ...
        logger.error(f"Error in combined operation: {e!s}")
        raise HTTPException(status_code=500, detail=f"Combined operation error: {e!s}") from e
...
async def classify_and_send_report_mock(
    ...
    try:
        ...
        logger.error(f"Error in mock combined operation: {e!s}")
        raise HTTPException(status_code=500, detail=f"Mock combined operation error: {e!s}") from e
