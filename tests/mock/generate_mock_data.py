...
async def generate_mock_ticket():
    ...
    return {
        ...
        "screenshot_url": f"https://example.com/screenshots/issue-{ticket_number}.png"
...
async def main():
    ...
    if not await check_api_health():
        ...
        print(
            "   cd backend && python -m uvicorn src.ticket_assistant.api.main:app --reload --host 0.0.0.0 --port 8000"
        )
        return
    ...
    async with httpx.AsyncClient() as client:
        ...
        if initial_stats:
            print(
                f"📊 Initial database state: {initial_stats['total_tickets']} tickets"
            )
        ...
        for i in range(TOTAL_TICKETS):
            ...
            print(
                f"  Creating ticket {i + 1}/{TOTAL_TICKETS}: {ticket_data['name'][:50]}..."
            )
            ...
            if result and result.get("success"):
                ...
                print(
                    f"    ✅ Created ticket {result['ticket']['id']} - {result['classification']['department'].upper()}/{result['classification']['severity'].upper()}"
                )
            else:
        ...
        if final_stats:
            ...
            print(
                f"   Average resolution time: {final_stats['average_resolution_time']} hours"
            )
            print(
                f"   Classification accuracy: {final_stats['classification_accuracy']}%"
            )
        ...
        if successful_tickets:
            ...
            for ticket in successful_tickets[:5]:  # Show first 5
                ...
                print(
                    f"   • {t['name'][:40]}... [{c['department'].upper()}/{c['severity'].upper()}]"
                )
