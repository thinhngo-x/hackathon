...
Tests database connection, schema, data integrity, and operations
"""
...
async def test_table_existence():
    ...
    try:
        async with AsyncSessionLocal() as session:
            # Check tickets table
            result = await session.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='tickets'"))
            if result.fetchone():
            ...
            result = await session.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='classifications'"))
            if result.fetchone():
        ...
        async with AsyncSessionLocal() as session:
            ...
            if tickets:
                ...
                for ticket in tickets:
                    print(f"      • {ticket[1][:40]}... [{ticket[2].upper()}/{ticket[3].upper()}] ({ticket[4]})")
            else:
...
async def test_database_schema():
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            expected_columns = ['id', 'name', 'description', 'error_message', 'department', 
                              'severity', 'status', 'assignee', 'screenshot_url', 
            ...
            expected_class_columns = ['id', 'ticket_id', 'confidence', 'reasoning', 
