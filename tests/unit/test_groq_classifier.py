...
from ticket_assistant.core.models import ClassificationRequest, Department, ErrorSeverity
...
class TestGroqClassifier:
            ...
            "choices": [
                {
                    "message": {
                        "content": '''
                        {
                            ...
                            "severity": "high",
                            "confidence": 0.95,
                            "reasoning": "The error indicates a database connection timeout, which is a database infrastructure issue requiring immediate attention.",
                            "suggested_actions": ["Check database server status", "Verify network connectivity", "Review connection pool settings"]
                        }
                        '''
                    }
                }
    ...
    def test_groq_classifier_initialization_with_api_key(self):
        ...
        classifier = GroqClassifier(api_key="test-api-key")
    ...
    async def test_classify_error_success(self, mock_groq_response, sample_classification_request):
        ...
        with patch('groq.Groq') as mock_groq:
            ...
            mock_client.chat.completions.create.return_value = type('Response', (), mock_groq_response)()
    ...
    def test_build_classification_prompt_minimal(self):
        ...
        prompt = classifier._build_classification_prompt(
            error_description="Test error only"
        )
    ...
    async def test_classify_error_different_departments(self):
        ...
        for case in test_cases:
            ...
            with patch('groq.Groq') as mock_groq:
                mock_client = MagicMock()
                mock_groq.return_value = mock_client
                mock_client.chat.completions.create.return_value = type('Response', (), mock_response)()
