...
from ticket_assistant.database.connection import init_db, AsyncSessionLocal
...
async def create_sample_tickets(session, count: int = 50):
    ...
    for i in range(count):
        ...
        base_ticket = choice(SAMPLE_TICKETS)
        
        # Generate realistic timestamps
        days_ago = randint(1, 90)  # Tickets from last 90 days
        ...
        if status in ["resolved", "closed"]:
            ...
            resolution_hours = uniform(0.5, 168)  # 0.5 hours to 7 days
        ...
        ticket_data = {
            ...
            "updated_at": created_at + timedelta(hours=randint(0, 24)),
    ...
    for ticket in tickets:
        ...
        if uniform(0, 1) < 0.7:
            classification = Classification(
                ...
                reasoning=choice(sample_reasonings),
                suggested_actions=json.dumps(choice(sample_actions)),
                created_at=ticket.created_at + timedelta(minutes=randint(1, 30)),
            )
