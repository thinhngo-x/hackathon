...
async def send_report(report: ReportRequest, service: ReportService = Depends(get_report_service)) -> ReportResponse:
    ...
    try:
        ...
        logger.error(f"Error processing report: {e!s}")
...
async def send_mock_report(
    ...
    try:
        ...
        logger.error(f"Error in mock report: {e!s}")
        raise HTTPException(status_code=500, detail=f"Mock report error: {e!s}") from e
